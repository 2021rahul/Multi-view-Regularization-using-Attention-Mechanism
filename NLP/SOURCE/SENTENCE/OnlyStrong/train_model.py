...
with tf.name_scope('data'):
    X = tf.placeholder(tf.int32, [None, config.max_sentence_length], name="inputs")
...
with tf.variable_scope("optimizer", reuse=tf.AUTO_REUSE):
    optimizer = tf.train.AdamOptimizer(config.SENTENCE_OnlyStrong_learning_rate).minimize(loss, global_step)
# %%
print("TRAIN MODEL")
saver = tf.train.Saver()
merged_summary_op = tf.summary.merge_all()
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter(os.path.join(config.MODEL_DIR, "SENTENCE", "OnlyStrong"), sess.graph)
    sess.run(tf.global_variables_initializer())
    sess.run(tf.local_variables_initializer())
    sess.run(tf_embedding_init, feed_dict={tf_embedding_placeholder: index_to_embedding})
    for i in range(config.SENTENCE_OnlyStrong_n_epochs):
    ...
    save_path = saver.save(sess, os.path.join(config.MODEL_DIR, "SENTENCE", "OnlyStrong", "model.ckpt"))
