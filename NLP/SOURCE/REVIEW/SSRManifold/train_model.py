...
train_data_unlabeled = np.load(os.path.join(config.NUMPY_DIR, "consistency_data_coarse.npy"))
...
with tf.name_scope('data'):
    X = tf.placeholder(tf.int32, [None, num_features], name="inputs")
    X_unlabeled = tf.placeholder(tf.int32, [None, num_features], name="inputs_unlabeled")
...
with tf.name_scope("loss_function"):
    ...
    r = tf.reshape(tf.reduce_sum(state_series_unlabelled[:,-1,:]*state_series_unlabelled[:,-1,:], 1), [-1, 1])
    x_dist = r - 2*tf.matmul(state_series_unlabelled[:,-1,:], tf.transpose(state_series_unlabelled[:,-1,:])) + tf.transpose(r)
    corr_loss = tf.reduce_mean(-tf.contrib.metrics.streaming_pearson_correlation(tf.reshape(fx_diff, [-1,1]), tf.reshape(x_dist, [-1,1]))[1])
    loss = (squared_loss + config.REVIEW_SSRManifold_reg_param*corr_loss)/(1+config.REVIEW_SSRManifold_reg_param)
tf.summary.scalar('loss', loss)

global_step = tf.Variable(0, name='global_step', trainable=False)
with tf.variable_scope("optimizer", reuse=tf.AUTO_REUSE):
    optimizer = tf.train.AdamOptimizer(config.REVIEW_SSRManifold_learning_rate).minimize(loss, global_step)
# %%
print("TRAIN MODEL")
saver = tf.train.Saver()
merged_summary_op = tf.summary.merge_all()
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter(os.path.join(config.MODEL_DIR, "REVIEW", "SSRManifold"), sess.graph)
    sess.run(tf.global_variables_initializer())
    sess.run(tf.local_variables_initializer())
    sess.run(tf_embedding_init, feed_dict={tf_embedding_placeholder: index_to_embedding})
    k=0
    for i in range(config.REVIEW_SSRManifold_n_epochs):
        data = train_data[:,:num_features]
        label = np.reshape(train_data[:, -1], [-1,1])

        if k*config.REVIEW_SSRManifold_batch_size>len(train_data_unlabeled) or (k+1)*config.REVIEW_SSRManifold_batch_size>len(train_data_unlabeled):
            k = 0
        data_unlabeled = train_data_unlabeled[(k*config.REVIEW_SSRManifold_batch_size)%len(train_data_unlabeled):((k+1)*config.REVIEW_SSRManifold_batch_size)%len(train_data_unlabeled), :]
    ...
    save_path = saver.save(sess, os.path.join(config.MODEL_DIR, "REVIEW", "SSRManifold", "model.ckpt"))
