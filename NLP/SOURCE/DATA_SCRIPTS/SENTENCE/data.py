...
#%%
with open(os.path.join(config.SENTENCE_DIR, "train_data.txt"), "r", encoding="utf-8") as f_data:
    data = f_data.readlines()
with open(os.path.join(config.SENTENCE_DIR, "train_label.txt"), "r", encoding="utf-8") as f_label:
...
np.save(os.path.join(config.NUMPY_DIR, "sentence_train_data"), train_data)

with open(os.path.join(config.SENTENCE_DIR, "test_data.txt"), "r", encoding="utf-8") as f_data:
    data = f_data.readlines()
with open(os.path.join(config.SENTENCE_DIR, "test_label.txt"), "r", encoding="utf-8") as f_label:
    label = f_label.readlines()
test_data = []
for i in range(len(label)):
    word_indexes = [word_to_index[w] for w in data[i].split()]
...
np.save(os.path.join(config.NUMPY_DIR, "sentence_test_data"), test_data)
