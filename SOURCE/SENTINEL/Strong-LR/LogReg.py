...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 16:02:05 2019
...
train_data = np.load(os.path.join(data_dir, "sentinel_train_data_"+str(num_train_samples)+".npy"))
...
with tf.name_scope('data'):
    X = tf.placeholder(tf.float32, [None, 12], name="inputs")
...
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter("../../MODEL/SENTINEL/LogReg", sess.graph)
    ...
    save_path = saver.save(sess, "../../MODEL/SENTINEL/LogReg/"+REGION+"_"+str(num_train_samples)+".CKPT")
# %%
# TEST MODEL ON TRAIN DATA
with tf.Session() as sess:
    saver.restore(sess, "../../MODEL/SENTINEL/LogReg/"+REGION+"_"+str(num_train_samples)+".CKPT")
    data = train_data[:, :12]
    feed_dict = {X: data}
