...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 16:41:23 2019
...
train_data_labeled = np.load(os.path.join(data_dir, "sentinel_train_data_"+str(num_train_samples)+".npy"))
train_data_unlabeled = np.load(os.path.join(data_dir, "sentinel_consistency_data.npy"))
test_data = np.load(os.path.join(data_dir, "sentinel_test_data.npy"))
# %%
...
with tf.name_scope('data'):
    X_labeled = tf.placeholder(tf.float32, [None, 12], name="inputs_labeled")
    X_unlabeled = tf.placeholder(tf.float32, [None, 12], name="inputs_unlabeled")
    Y = tf.placeholder(tf.float32, [None, 1], name="labels")

with tf.variable_scope("Variables", reuse=tf.AUTO_REUSE):
    W = tf.get_variable("Weights", [12, 1], initializer=tf.contrib.layers.xavier_initializer())
...
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter("../../MODEL/SENTINEL/SemiSupervised_LogReg", sess.graph)
    sess.run(tf.global_variables_initializer())
    sess.run(tf.local_variables_initializer())
    k=0
    for i in range(n_epochs):
        data_labeled = train_data_labeled[:, :12]
    ...
    save_path = saver.save(sess, "../../MODEL/SENTINEL/SemiSupervised_LogReg/"+REGION+"_"+str(num_train_samples)+".CKPT")
# %%
# TEST MODEL ON TRAIN DATA
with tf.Session() as sess:
    saver.restore(sess, "../../MODEL/SENTINEL/SemiSupervised_LogReg/"+REGION+"_"+str(num_train_samples)+".CKPT")
    data = train_data_labeled[:, :12]
