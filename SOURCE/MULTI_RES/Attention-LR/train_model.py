...
ind = list(itertools.chain.from_iterable(itertools.repeat(x, 9) for x in range(config.batch_consistency)))
...
Z_fine_consistency = tf.matmul(X_attention_consistency, W_fine, name="multiply_weights")
...
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter(os.path.join(config.MODEL_DIR, "MULTI_RES", "Attention-LR"), sess.graph)
    ...
    save_path = saver.save(sess, os.path.join(config.MODEL_DIR, "MULTI_RES", "Attention-LR", "model.ckpt"))
