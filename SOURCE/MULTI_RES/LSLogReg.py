...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 16:54:16 2019
...
train_data_fine = np.load(os.path.join(data_dir, "sentinel_train_data_"+str(num_train_samples)+".npy"))
train_data_coarse = np.load(os.path.join(data_dir, "landsat_train_data_"+str(num_train_samples)+".npy"))
...
for i in range(batch_consistency):
    batch_pred_fine_consistency = pred_fine_consistency[i*9:(i+1)*9]
    softmax_batch_pred_fine_consistency = tf.pow(x, batch_pred_fine_consistency)/tf.reduce_sum(tf.pow(x, batch_pred_fine_consistency))
    coarse_pred_fine_consistency.append(tf.matmul(softmax_batch_pred_fine_consistency, batch_pred_fine_consistency, transpose_a=True, transpose_b=False))
...
with tf.name_scope("loss_function"):
    ...
    loss_consistency = tf.reduce_mean(tf.squared_difference(coarse_pred_fine_consistency, pred_coarse_consistency))
    loss = (lambda1*loss_fine + lambda2*loss_coarse + lambda3*loss_consistency)
...
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter("../../MODEL/SENTINEL_LANDSAT", sess.graph)
    sess.run(tf.global_variables_initializer())
    sess.run(tf.local_variables_initializer())
    k=0
    for i in range(n_epochs):
        data_fine = train_data_fine[:, :12]
        label_fine = np.reshape(train_data_fine[:, -1], [-1,1])
        data_coarse = train_data_coarse[:, :11]
    ...
    save_path = saver.save(sess, "../../MODEL/SENTINEL_LANDSAT/"+REGION+"_"+str(num_train_samples)+".CKPT")
# %%
# TEST MODEL ON LANDSAT TRAIN DATA
with tf.Session() as sess:
    saver.restore(sess, "../../MODEL/SENTINEL_LANDSAT/"+REGION+"_"+str(num_train_samples)+".CKPT")
    data = train_data_coarse[:, :11]
