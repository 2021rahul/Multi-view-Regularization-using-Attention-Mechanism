...
osm_im = Image.open(os.path.join(OSM_DIR, "LANDSAT_label.tif"))
...
data_0 = np.concatenate((landsat, sentinel, np.zeros((num_train_samples, 1))), axis = 1)
...
for index in range(num_train_samples):
    ...
    landsat[index,:] = landsat_data_region[landsat_i, landsat_j, :]
    sentinel[index, :] = np.mean(np.reshape(sentinel_data_region[landsat_i*3:(landsat_i+1)*3, landsat_j*3:(landsat_j+1)*3, :], (-1,12)), axis = 0)
data_1 = np.concatenate((landsat, sentinel, np.ones((num_train_samples, 1))), axis = 1)

train_data = np.concatenate((data_0, data_1))
index = random.sample(range(2*num_train_samples), 2*num_train_samples)
train_data = train_data[index]
np.save(os.path.join(OUT_DIR, "landsat_train_data_"+str(2*num_train_samples)), train_data)
##%%
