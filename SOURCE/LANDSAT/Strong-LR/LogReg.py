...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 26 13:51:44 2019
...
with tf.name_scope('data'):
    X = tf.placeholder(tf.float32, [None, 11], name="inputs")
...
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter("../../MODEL/LANDSAT/LogReg", sess.graph)
    ...
    save_path = saver.save(sess, "../../MODEL/LANDSAT/LogReg/"+REGION+"_"+str(num_train_samples)+".CKPT")
# %%
# TEST MODEL ON TRAIN DATA
with tf.Session() as sess:
    saver.restore(sess, "../../MODEL/LANDSAT/LogReg/"+REGION+"_"+str(num_train_samples)+".CKPT")
    data = train_data[:, :11]
    feed_dict = {X: data}
