...
train_data_unlabeled = np.load(os.path.join(config.NUMPY_DIR, "landsat_consistency_data.npy"))
num_features = 11
# %%
print("BUILD MODEL")
...
with tf.name_scope("loss_function"):
    ...
    loss = (squared_loss + config.LANDSAT_SSRManifold_reg_param*corr_loss)/(1+config.LANDSAT_SSRManifold_reg_param)
tf.summary.scalar('loss', loss)

global_step = tf.Variable(0, name='global_step', trainable=False)
with tf.variable_scope("optimizer", reuse=tf.AUTO_REUSE):
    optimizer = tf.train.AdamOptimizer(config.LANDSAT_SSRManifold_learning_rate).minimize(loss, global_step)
# %%
# TRAIN MODEL
saver = tf.train.Saver()
merged_summary_op = tf.summary.merge_all()
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter(os.path.join(config.MODEL_DIR, "LANDSAT", "SSRManifold"), sess.graph)
    sess.run(tf.global_variables_initializer())
    sess.run(tf.local_variables_initializer())
    k=0
    for i in range(config.LANDSAT_SSRManifold_n_epochs):
        data = train_data[:, :num_features]
        label = np.reshape(train_data[:, -1], [-1,1])

        if k*config.LANDSAT_SSRManifold_batch_size>len(train_data_unlabeled) or (k+1)*config.LANDSAT_SSRManifold_batch_size>len(train_data_unlabeled):
            k = 0
        data_unlabeled = train_data_unlabeled[(k*config.LANDSAT_SSRManifold_batch_size)%len(train_data_unlabeled):((k+1)*config.LANDSAT_SSRManifold_batch_size)%len(train_data_unlabeled), :]
    ...
    save_path = saver.save(sess, os.path.join(config.MODEL_DIR, "LANDSAT", "SSRManifold", "model.ckpt"))
