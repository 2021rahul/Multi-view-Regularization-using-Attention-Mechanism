...
# %%
print("LOAD DATA")
validate_data_fine = np.load(os.path.join(config.NUMPY_DIR, "sentinel_validate_data.npy"))
validate_data_coarse = np.load(os.path.join(config.NUMPY_DIR, "landsat_validate_data.npy"))
...
with tf.Session() as sess:
    saver.restore(sess, os.path.join(config.MODEL_DIR, "MULTI_RES", "MIL", "model.ckpt"))
    data_fine = validate_data_fine[:,:num_features_fine]
    data_coarse = validate_data_coarse[:,:num_features_coarse]
